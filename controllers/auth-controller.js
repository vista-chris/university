...
//sending mail
const main = async (fname, email, link) => {
...
const signup = async (req, res) => {
    ...
    try {
        ...
        const resetLink = `http://localhost:3000/reset/user/${id}/${token}`;
...
const forgotStudentPassword = async (req, res) => {
    ...
    try {
        ...
        const payload = { email, id };
        const token = jwt.sign(payload, secret, { expiresIn: '15m' });
        const resetLink = `http://localhost:3000/reset/student/${id}/${token}`;
        main(fname, email, resetLink);
        res.status(201).json({ success: 'Your password reset link has been sent. Please check your email' });
    } catch (error) {
        const err = errorHandler(error);
    ...
    try {
        ...
        if (student) {
            ...
            if (auth) {
                ...
                return res.json({ success: 'Your password has been updated!' });
...
const addStudent = async (req, res) => {
    ...
    const password = String(generateCode());
    const adm = String(generateCode());
