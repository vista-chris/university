...
app.controller("myCtrl", ($scope, $http, $timeout) => {
    ...
    $scope.modal_page_position = (page_number) => {
        $scope.current_modal_grid = page_number;
    };
    ...
    $scope.updatePassword = () => {
        ...
        $http.post("/update/password", data).then((res) => {
            $("#btn-updatePassword").html("Update Password").removeClass("disabled");
            res.data.success
            ...
            $http.post(`/user/update/user/${$scope.id}`, data).then((res) => {
                ...
                if (res.data.success) {
                    ...
                    $scope.id =
                        ...
                        $scope.category =
                        null;
                    document.getElementById("delete-user-account").hidden = true;
                    $scope.successMsg("Updated!", res.data.success);
                } else {
    ...
    $scope.updateUser = (
        ...
        if (document.getElementById("delete-user-account")) {
            document.getElementById("delete-user-account").hidden = false;
        }
    ...
    $scope.fetchUsers = () => {
        $http.get("/user/fetch/users").then((res) => {
            ...
            if (document.getElementById("users-table")) {
                ...
                document.getElementById("users-table-loader").hidden = true;
            }
        });
    };
    ...
    $scope.deleteUser = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $("#btn-deleteUser")
                    .html(
                        '<span class="spinner-border spinner-border-sm mr-2" role="status" aria-hidden="true"></span> Deleting Account...'
                    )
                    .addClass("disabled");
                $http.post(`/user/delete/user/${$scope.id}`).then((res) => {
                    $("#btn-deleteUser").html("Delete Account").removeClass("disabled");
                    if (res.data.success) {
                        $scope.fetchUsers();
                        $scope.title = "Add";
                        $scope.id =
    ...
    $scope.deactivateUser = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $("#btn-deactivateUser")
                    .html(
                        '<span class="spinner-border spinner-border-sm mr-2" role="status" aria-hidden="true"></span> Deactivating...'
                    )
                    .addClass("disabled");
                $http.post(`/user/deactivate/user/${$scope.id}`).then((res) => {
                    $("#btn-deactivateUser").html("Deactivate").removeClass("disabled");
                    if (res.data.success) {
                        $scope.fetchUsers();
                        $scope.title = "Add";
                        $scope.id =
                        ...
                        document.getElementById("delete-user-account").hidden = true;
                        $scope.successMsg("Deactivated!", res.data.success);
                    } else {
                        $scope.errorMsg("", res.data.err);
                    }
                });
    ...
    $scope.activateUser = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $("#btn-activateUser")
                    .html(
                        '<span class="spinner-border spinner-border-sm mr-2" role="status" aria-hidden="true"></span> Activating...'
                    )
                    .addClass("disabled");
                $http.post(`/user/activate/user/${$scope.id}`).then((res) => {
                    $("#btn-activateUser").html("Activate").removeClass("disabled");
                    if (res.data.success) {
                        $scope.fetchUsers();
                        $scope.title = "Add";
                        $scope.id =
                        ...
                        document.getElementById("delete-user-account").hidden = true;
                        $scope.successMsg("Activated!", res.data.success);
                    } else {
                        $scope.errorMsg("", res.data.err);
                    }
                });
    ...
    $scope.fetchStudents = () => {
        $http.get("/user/students/fetch").then((res) => {
            ...
            if (document.getElementById("students-table") && document.getElementById('studentscsv')) {
                document.getElementById("students-table").hidden = false;
                document.getElementById('studentscsv').append(link);
                document.getElementById("students-table-loader").hidden = true;
            }
        });
    };
    ...
    $scope.fetchCourses = () => {
        $http.get("/user/fetch/courses").then((res) => {
            ...
            if (document.getElementById("courses-table")) {
                ...
                document.getElementById("courses-table-loader").hidden = true;
            }
        });
    };
    ...
    $scope.fetchUnits = () => {
        $http.get("/fetch/units").then((res) => {
            ...
            if (document.getElementById("units-table") && document.getElementById('unitscsv')) {
                document.getElementById("units-table").hidden = false;
                document.getElementById('unitscsv').append(link);
                document.getElementById("units-table-loader").hidden = true;
            }
        });
    };
    ...
    $scope.fetchPeriod = () => {
        $http.get("/user/fetch/period").then((res) => {
            ...
            $scope.initialPeriod = res.data[0].name;
        });
    };

    //apply selected units
    $scope.applySelectedUnits = () => {
        const period = $scope.initialPeriod;
        const unit = [];
        $('input[name="unitCheck"]:checkbox:checked').each(function (i) {
            unit[i] = $(this).val();
        });
        ...
        Swal.fire({
            title: "Are you sure?",
            text: `The selected units will be applied on offer for the current period ${$scope.initialPeriod}.`,
            icon: "warning",
            showCancelButton: true,
    ...
    $scope.fetchOfferedUnits = () => {
        $http.get("/fetch/offered/units").then((res) => {
            ...
            if (document.getElementById("offered-units-table")) {
                ...
                document.getElementById("offered-units-loader").hidden = true;
            }
        });
    };
    ...
    $scope.fetchAttendanceRecords = () => {
        $http.get(`/attendance/records`).then((res) => {
            ...
            if (document.getElementById("attendance-table")) {
                ...
                document.getElementById("attendance-table-loader").hidden = true;
            }
        });
    };
    ...
    $scope.fetchUnitAttendance = (id, code, unit, date) => {
        ...
        $http.post(`/unit/attendance/${id}`).then((res) => {
            ...
            if ($scope.unitAttendance.length < 1) {
                $scope.promptMsg('', 'There are no records/registered students for this unit. Attendance details will be displayed after students registration')
            } else {
    ...
    $scope.fetchRegUnits = () => {
        $http.post(`/student/fetch/reg/units/${$scope.userId}`).then((res) => {
            ...
            if (document.getElementById("reg-units-table")) {
                ...
                document.getElementById("reg-units-loader").hidden = true;
            }
        });
    };
    ...
    $scope.fetchTimetable = () => {
        $http.get("/user/fetch/timetable").then((res) => {
            ...
            if (document.getElementById("timetable")) {
                ...
                document.getElementById("timetable-loader").hidden = true;
            }
        });
    };
    ...
    $scope.fetchStructure = () => {
        $http.get("/user/fetch/structure").then((res) => {
            ...
            if (document.getElementById("structure-table")) {
                ...
                document.getElementById("structure-table-loader").hidden = true;
            }
        });
    };
    ...
    $scope.fetchFeeStatements = () => {
        $http.get(`/student/fetch/fee/statements/${$scope.userId}`).then((res) => {
            ...
            if (document.getElementById("fee-statements-table")) {
                ...
                document.getElementById("fee-statements-table-loader").hidden = true;
            }
        });
    };
        ...
        $http
            ...
            .then((res) => {
                ...
                if (document.getElementById("accomodation-statements-table")) {
                    ...
                    document.getElementById("accomodation-statements-table-loader").hidden = true;
                }
            });
    };
